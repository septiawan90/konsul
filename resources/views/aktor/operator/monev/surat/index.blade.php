                    ...
                    <table class="table table-bordered table-hover table-striped">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="align-middle align-middle">{{ kapital($s->lpp->nama) }} {{ $s->lpp->alias ? '('.$s->lpp->alias.')' : '' }}</td>
                                <td class="text-center align-middle">{{ $s->nomor }}<br>{{ tanggal($s->tanggal) }}</td>
                                <td class="align-middle">{{ $s->tentang }}</td>
                                <td class="text-center align-middle">
                                    <form method="post" name="formStatus" action="{{ $link }}/{{ $judul }}/status/{{ $user_id }}/{{ $profil_id }}/{{ $profesi_id }}/{{ Crypt::encrypt($s->id) }}" onSubmit="return confirm('Anda yakin akan mengubah status surat dengan {{ $s->nomor }} {{ tanggal($s->tanggal) }} dari {{ kapital($s->lpp->nama) }} {{ $s->lpp->alias ? '('.$s->lpp->alias.')' : '' }} ini?')" class="form-inline ml-0 ml-md-3">
                                    {{ csrf_field() }}
                                    {{ method_field('PUT') }}

                                        <div class="mt-1 btn-group btn-group-toggle" data-toggle="buttons">
                                            <label class="btn btn-xs {{ $s->status == 'setuju' ? 'btn-success active' : 'btn-outline-secondary' }}">
                                                <input type="radio" name="status" id="option1" autocomplete="off" value="setuju" onChange="this.form.submit()" checked="{{ $s->status == 'setuju' ? 'checked' : '' }}">Setuju
                                            </label>
                                            <label class="btn btn-xs {{ $s->status == 'tolak' ? 'btn-danger active' : 'btn-outline-secondary' }}">
                                                <input type="radio" name="status" id="option2" autocomplete="off" value="tolak" onChange="this.form.submit()" checked="{{ $s->status == 'setuju' ? 'checked' : '' }}">Tolak
                                            </label>
                                        </div>
                                ...
                                <td class="text-center align-middle">
                                    <a href="{{ $link }}/kegiatan/{{ $user_id }}/{{ $profil_id }}/{{ $profesi_id }}/{{ Crypt::encrypt($s->id) }}" class="btn btn-xs btn-outline-success" data-placement="bottom" data-toggle="tooltip" data-original-title="Daftar Kegiatan"><i class="fa fa-plane"></i> {{ Statistik::hitungKegiatan($s->id) }}</a>
                                </td>
                                <td class="text-center align-middle">
                                    <a href="{{ $link }}/{{ $judul }}/unduh/{{ $user_id }}/{{ $profil_id }}/{{ $profesi_id }}/{{Crypt::encrypt($s->id)}}" class="btn btn-xs btn-outline-primary" data-placement="bottom" data-toggle="tooltip" data-original-title="Unduh"><i class="fa fa-download"></i></a>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
