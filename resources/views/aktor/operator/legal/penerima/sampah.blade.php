                    ...
                    <table class="table table-bordered table-hover table-striped">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="row">
                                        <div class="col-sm-10">{{ $d->nomor }}</div>
                                        <div class="col-sm-2">@if($d->file)
                                            <a href="{{ $link }}/unduh/{{Crypt::encrypt($d->id)}}" class="btn btn-xs btn-success" data-placement="top" data-toggle="tooltip" data-original-title="Unduh"><i class="fa fa-download"></i></a>
                                        @endif</div>
                                    </div>
                                </td>
                                <td class="text-center">{{ tanggal($d->tanggal) }}</td>
                                <td>{{ $d->tentang }}</td>
                                <td>{{ isset($d->profesi->nama) ? $d->profesi->nama : "" }}</td>
                                <td class="text-center">{{ tanggal($d->kadaluarsa) }}</td>
                                <td class="text-center">
                                    <div class="btn-group">
                                    <a href="{{ $link }}/kembalikan/{{ Crypt::encrypt($d->id) }}" class="btn btn-xs btn-success" data-placement="top" data-toggle="tooltip" data-original-title="Kembalikan"><i class="fa fa-recycle"></i></a>
                                    <a href="{{ $link }}/hapus_permanen/{{ Crypt::encrypt($d->id) }}" class="btn btn-xs btn-danger" onclick="return confirm('Yakin hapus permanen sk {{ $d->nomor }} ini?')" data-placement="top" data-toggle="tooltip" data-original-title="Hapus Permanen"><i class="fa fa-trash-alt"></i></a>
                                    </div>
                                </td>
                            </tr>
                            @endforeach
